CREATE SEQUENCE IF NOT EXISTS countries_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS countries (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE countries_id_seq PRIMARY KEY,
    name varchar(75)
);

CREATE SEQUENCE IF NOT EXISTS provinces_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS provinces (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE provinces_id_seq PRIMARY KEY,
    name varchar(75),
    countryId INTEGER REFERENCES countries(id) ON DELETE SET NULL
);

CREATE SEQUENCE IF NOT EXISTS cities_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS cities (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE cities_id_seq PRIMARY KEY,
    name varchar(75),
    countryId INTEGER REFERENCES countries(id) ON DELETE SET NULL,
    provinceId INTEGER REFERENCES countries(id) ON DELETE SET NULL
);

CREATE SEQUENCE IF NOT EXISTS neighbourhoods_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS neighbourhoods (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE neighbourhoods_id_seq PRIMARY KEY,
    name varchar(75),
    cityId INTEGER REFERENCES cities(id) ON DELETE SET NULL
);

CREATE SEQUENCE IF NOT EXISTS universities_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS universities (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE universities_id_seq PRIMARY KEY,
    name varchar(75)
);

CREATE SEQUENCE IF NOT EXISTS careers_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS careers (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE careers_id_seq PRIMARY KEY,
    name varchar(75)
);

CREATE SEQUENCE IF NOT EXISTS users_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS users (
    id BIGINT GENERATED BY DEFAULT AS SEQUENCE users_id_seq PRIMARY KEY,
    email varchar(250),
    username varchar(250),
    passwordHash varchar(300),
    name varchar(50),
    lastName varchar(100),
    birthDate date,
    gender varchar(50),
    bio varchar(1000),
    contactNumber varchar(25),
    universityId INTEGER REFERENCES universities(id),
    careerId INTEGER REFERENCES careers(id),
    role VARCHAR(50)
);

CREATE SEQUENCE IF NOT EXISTS properties_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS properties (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE properties_id_seq PRIMARY KEY,
    description varchar(3000),
    caption varchar(250),
    image varchar(300),
    propertyType varchar(100),
    neighbourhoodId integer references neighbourhoods(id) ON DELETE SET NULL,
    privacyLevel boolean,
    capacity integer,
    price float,
    mainimageid integer, --THIS SHOULD BE A FOREIGN KEY! BUT THE ADD COLUMN IF NOT EXISTS WASN'T WORKING TODO FIX
    ownerId INTEGER REFERENCES  users(id) ON DELETE CASCADE,
    availability varchar(100) NOT NULL
);

CREATE SEQUENCE IF NOT EXISTS interests_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS interests (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE interests_id_seq PRIMARY KEY,
    propertyId INTEGER REFERENCES properties(id),
    userId INTEGER REFERENCES users(id)
);

CREATE SEQUENCE IF NOT EXISTS rules_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS rules (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE rules_id_seq PRIMARY KEY,
    name varchar(250)
);

CREATE SEQUENCE IF NOT EXISTS propertyRules_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS propertyRules (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE propertyRules_id_seq PRIMARY KEY,
    propertyId INTEGER REFERENCES properties(id),
    ruleId INTEGER REFERENCES rules(id)
);

CREATE SEQUENCE IF NOT EXISTS services_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS services (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE services_id_seq PRIMARY KEY,
    name varchar(100)
);

CREATE SEQUENCE IF NOT EXISTS propertyServices_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS propertyServices (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE propertyServices_id_seq PRIMARY KEY,
    propertyId INTEGER REFERENCES properties(id) ON DELETE CASCADE,
    serviceId INTEGER REFERENCES services(id) ON DELETE CASCADE
);

CREATE SEQUENCE IF NOT EXISTS images_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS images (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE images_id_seq PRIMARY KEY,
    propertyId INTEGER REFERENCES properties(id) ON DELETE CASCADE,
    image blob
);

CREATE SEQUENCE IF NOT EXISTS notifications_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS notifications (
    id INTEGER GENERATED BY DEFAULT AS SEQUENCE notifications_id_seq PRIMARY KEY,
    userId INTEGER REFERENCES users(id) ON DELETE CASCADE,
    subjectCode VARCHAR(250),
    textCode VARCHAR(250),
    link VARCHAR(250),
    state VARCHAR(75)
);

CREATE SEQUENCE IF NOT EXISTS proposals_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS proposals (
  id INTEGER GENERATED BY DEFAULT AS SEQUENCE proposals_id_seq PRIMARY KEY,
  propertyId INTEGER REFERENCES properties(id) ON DELETE CASCADE,
  creatorId INTEGER REFERENCES users(id) ON DELETE CASCADE,
  state VARCHAR(100)
);

CREATE SEQUENCE IF NOT EXISTS userProposals_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;

CREATE TABLE IF NOT EXISTS  userProposals (
  id INTEGER GENERATED BY DEFAULT AS SEQUENCE userProposals_id_seq PRIMARY KEY,
  userId INTEGER REFERENCES users(id) ON DELETE CASCADE,
  proposalId INTEGER REFERENCES proposals(id) ON DELETE CASCADE,
  state integer
);
-- Populating test database to check access

INSERT INTO countries(
    name)
SELECT * FROM (VALUES ('Argentina'))
WHERE NOT EXISTS (SELECT * FROM countries WHERE id=1);

INSERT INTO cities(
    name, countryid, provinceid)
SELECT * FROM (VALUES ('CABA', 1, null))
WHERE NOT EXISTS (SELECT * FROM cities WHERE id=1);

INSERT INTO neighbourhoods(
    name, cityid)
SELECT * FROM (VALUES ('Palermo', 1))
WHERE NOT EXISTS (SELECT * FROM neighbourhoods WHERE id=1);

INSERT INTO universities(name)
SELECT * FROM (VALUES('ITBA'))
WHERE NOT EXISTS (SELECT * FROM universities WHERE id=1);

INSERT INTO careers(name)
SELECT * FROM (VALUES('Ingenieria en Testing'))
WHERE NOT EXISTS (SELECT * FROM careers WHERE id=1);

INSERT INTO users (
    email, username, passwordhash, name, lastname, birthdate, gender, bio, contactnumber, universityid, careerid, role)
SELECT * FROM (VALUES ('johnTester@gmail.com', 'John_Tester', 'HDSASDKF', 'John', 'Tester', DATE '1996-07-12', 'MALE', 'RE capo', 1167567280, 1, 1, 'ROLE_GUEST'))
WHERE NOT EXISTS (SELECT * FROM users WHERE id=1);

INSERT INTO images(
    propertyid, image)
SELECT * FROM (VALUES (null, decode('', 'base64', '')))
WHERE NOT EXISTS (SELECT * FROM images WHERE id=1);


INSERT INTO properties(
    description, caption, propertytype, neighbourhoodid, privacylevel, capacity, price, mainimageid, ownerId, availability)
SELECT * FROM (VALUES ('posta que el mejor depto', 'el mejor depto', 'APARTMENT', 1, true, 5, 100, 1, 1, 'AVAILABLE'))
WHERE NOT EXISTS (SELECT * FROM properties WHERE id=1);

INSERT INTO images(
    propertyid, image)
SELECT * FROM (VALUES (1, decode('', 'base64', '')))
WHERE NOT EXISTS (SELECT * FROM images WHERE id=2);

INSERT INTO properties(
    description, caption, propertytype, neighbourhoodid, privacylevel, capacity, price, mainimageid, ownerId, availability)
SELECT * FROM (VALUES ( 'posta que el mejor depto', 'el mejor depto', 'APARTMENT', 1, true, 5, 100, 1, 1, 'AVAILABLE'))
WHERE NOT EXISTS (SELECT * FROM properties WHERE id=2);

INSERT INTO properties(
    description, caption, propertytype, neighbourhoodid, privacylevel, capacity, price, mainimageid, ownerId, availability)
SELECT * FROM (VALUES ('posta que el mejor depto', 'el mejor depto', 'APARTMENT', 1, true, 5, 100, 1, 1, 'AVAILABLE'))
WHERE NOT EXISTS (SELECT * FROM properties WHERE id=3);

INSERT INTO properties(
     description, caption, propertytype, neighbourhoodid, privacylevel, capacity, price, mainimageid, ownerId, availability)
SELECT * FROM (VALUES ( 'posta que el mejor depto', 'el mejor depto', 'APARTMENT', 1, true, 5, 100, 1,1, 'AVAILABLE' ))
WHERE NOT EXISTS (SELECT * FROM properties WHERE id=4);

INSERT INTO properties(
    description, caption, propertytype, neighbourhoodid, privacylevel, capacity, price, mainimageid, ownerId, availability)
SELECT * FROM (VALUES ('posta que el mejor depto', 'el mejor depto', 'APARTMENT', 1, true, 5, 100, 1, 1, 'AVAILABLE'))
WHERE NOT EXISTS (SELECT * FROM properties WHERE id=5);

INSERT INTO properties(
    description, caption, propertytype, neighbourhoodid, privacylevel, capacity, price, mainimageid, ownerId, availability)
SELECT * FROM (VALUES ('posta que el mejor depto', 'el mejor depto', 'APARTMENT', 1, true, 5, 100, 1,1, 'AVAILABLE' ))
WHERE NOT EXISTS (SELECT * FROM properties WHERE id=6);

INSERT INTO properties(
    description, caption, propertytype, neighbourhoodid, privacylevel, capacity, price, mainimageid, ownerId, availability)
SELECT * FROM (VALUES ('posta que el mejor depto', 'el mejor depto', 'APARTMENT', 1, true, 5, 100, 1, 1, 'AVAILABLE'))
WHERE NOT EXISTS (SELECT * FROM properties WHERE id=7);

INSERT INTO properties(
    description, caption, propertytype, neighbourhoodid, privacylevel, capacity, price, mainimageid, ownerId , availability)
    SELECT * FROM (VALUES ('posta que el mejor depto', 'el mejor depto', 'APARTMENT', 1, true, 5, 100, 1, 1, 'RENTED'))
WHERE NOT EXISTS (SELECT * FROM properties WHERE id=8);

INSERT INTO interests(id, propertyid, userid)
SELECT * FROM (VALUES(1, 1, 1))
WHERE NOT EXISTS (SELECT * FROM interests WHERE id = 1);

INSERT INTO rules(name)
SELECT * FROM (VALUES('No fumar mientras se escriben tests'))
WHERE NOT EXISTS (SELECT * FROM rules WHERE id=1);

INSERT INTO propertyrules ( propertyid, ruleid)
SELECT * FROM (VALUES(1, 1))
WHERE NOT EXISTS (SELECT * FROM propertyrules WHERE id=1);


INSERT INTO services ( name)
SELECT * FROM (VALUES('Iluminación'))
WHERE NOT EXISTS (SELECT * FROM services WHERE id=1);

INSERT INTO propertyServices ( propertyId, serviceId)
SELECT * FROM (VALUES(1, 1))
WHERE NOT EXISTS (SELECT * FROM propertyservices WHERE id = 1);

INSERT INTO notifications (
    userId, subjectCode, textCode, link, state)
SELECT * FROM (VALUES( 1, 'sth', 'sth', '/proposals/44', 'UNREAD'))
WHERE NOT EXISTS (SELECT * FROM notifications WHERE id=1);

INSERT INTO notifications (
    userId, subjectCode, textCode, link, state)
SELECT * FROM (VALUES(1, 'sth', 'sth', '/proposals/44', 'READ'))
WHERE NOT EXISTS (SELECT * FROM notifications WHERE id=2);

INSERT INTO proposals(
  propertyId, creatorId, state)
SELECT * FROM (VALUES( 1, 1, 'SENT'))
WHERE NOT EXISTS (SELECT * FROM proposals WHERE id=1);

